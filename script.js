document.addEventListener('DOMContentLoaded', () => { // 첫 번째 DOMContentLoaded 시작
    ...
    if (navToggle && navMenu) { // 요소가 존재할 때만 이벤트 리스너 추가
    ...
    fetch('cal.json')
        ...
        .then(data => {
            ...
            if (noticesSection && !document.querySelector('.notice-pagination')) { // 페이지네이션 버튼이 없으면 추가
            ...
            // 페이지 이동 함수들을 전역 스코프에 추가 (HTML에서 onclick으로 호출되므로 필요)
